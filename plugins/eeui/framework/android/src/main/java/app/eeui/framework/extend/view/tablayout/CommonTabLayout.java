package app.eeui.framework.extend.view.tablayout;
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
        ...
        if (iconUrl != null) {
            if (isFontIcon(iconUrl)) {
                if (!iconUrl.contains("#") || !iconUrl.contains("rgb")) {
                    iconUrl += " " + eeuiColorUtils.int2Hex(isCurrent ? mTextSelectColor : mTextUnselectColor);
                }
    ...
    private int getImageViewIntrinsicHeight(CustomTabEntity tabEntity) {
        ...
        if (iconUrl != null) {
            return eeuiParse.parseInt(imageViewHeight.get(iconUrl));
        }else{
