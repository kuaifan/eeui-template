...
- (void)hh_pushViewController:(UIViewController *)viewController style:(AnimationStyle)style swipeFullBack:(BOOL)swipeFullBack
{
    ...
    if (swipeFullBack) {
        UIPanGestureRecognizer * fullScreenPan = [[UIPanGestureRecognizer alloc]initWithTarget:interaction action:NSSelectorFromString(@"edgePanAction:")];
        [viewController.view addGestureRecognizer:fullScreenPan];
    }else{
